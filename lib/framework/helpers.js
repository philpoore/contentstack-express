...
var find = function (query) {
    // return Promise.coroutine(function* () {
        ...
        if(data) {
            ...
            // needs to be sync
            data = _db.find(query, {}, function (error, result) {
                if(error)
                    reject(error);
            ...
            // needs to be sync
            data = _db(contentTypeUid, query).then(function (result) {
        ...
        if(data) {
            if(provider === 'FileSystem' || provider === 'nedb')
                _data = _.map(_.cloneDeep(data), '_data');
            else
                _data = _.cloneDeep(data); /* Custom providers */

            if(include_references && (provider === 'FileSystem' || provider === 'nedb' && provider !== 'cdn'))
                _data = includeReferences(data, language, references);
        }
    return _data;
