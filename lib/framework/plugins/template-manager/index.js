...
module.exports = function TemplateManager() {
    ...
    TemplateManager.templateExtends = function(engine, app) {
      ...
      if(module == "nunjucks") {
            ...
            env.addFilter('partial', function (partial, callback) {
                console.log(partial)
                // console.log('inside partial filter', helper.get);
                helper.get(partial).then(function (entry) {
                    console.log(entry)
                    callback();
    ...
    TemplateManager.beforePublish = function (data, next) {
        try {
            var scaffold = config.get('view.scaffold'),
                syncContentOnly = config.get('syncContentOnly');
            scaffold = (typeof scaffold === "boolean") ? scaffold : true;
            syncContentOnly = (typeof syncContentOnly === "boolean") ? syncContentOnly : true;
            if (data && data.entry && (data.entry.url || (data.content_type.options && data.content_type.options.is_page)) && data.content_type && data.content_type.uid && data.content_type.schema && data.content_type.schema.length && scaffold && !syncContentOnly) {
