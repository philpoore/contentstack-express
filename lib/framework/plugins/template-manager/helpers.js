...
if(provider === 'FileSystem' || provider === 'nedb' || provider === 'cdn') {
    ...
    console.log('custom provider partials path invoked');
...
module.exports = (function () {
    ...
    function get (partial, limit, language, includeReference) {
        return new Promise(function (resolve, reject) {
            var language = language || context.get('lang'),
                limit = limit || 1,
                data = InMemory.get(language, partial, {}, true);
            // console.log(partial, limit, language, includeReference)
            ...
            if(data || provider === 'FileSystem' || provider === 'nedb') {
                ...
                findCDN({"_content_type_uid": partial, "locale": language, "include_references": includeReference}).then(function (entry) {
                    if (entry && entry.length)
                        entry = (limit == 1) ? entry[0] : entry.slice(0, limit);
                    return resolve(entry);
                })
    ...
    function findCDN (query) {
        return new Promise(function (resolve, reject) {
            _db.find(query, {}, function (error, result) {
                console.log('CDN provider', error, result);
                if(error)
                    reject(error);
                data = result.entries;
                resolve(data);
