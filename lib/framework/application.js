...
app.listen = function () {
	...
	opt.skip = new RegExp("(^("+_.map(languages, "relative_url_prefix").join("|")+")|[.]json)$");
    ...
    * */
	...
	// Execute this, if content sync is false
	if(!config.get('mode.api'))
...
function respond() {
	return function (req, res, next) {
		if (req._contentstack.response_type !== 'json' && !config.get('mode.api')) {
			if (req._contentstack.template && req._contentstack.entry) {
				...
				res.render(req._contentstack.template, result, function (error, respose) {
					var result = _.cloneDeep(utils.context.getAll());
