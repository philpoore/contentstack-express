...
/**
 * loads path of assets for each language
 * @param  {Object} _assets  - Empty object on which the path's will be loaded
 * @return {Object}          - Object that as path to each `_asset.json` path with
 *                             its lang-code as its key
 */

function loadAssetPaths (_assets) {
    for (var i = 0, _i = languages.length; i < _i; i++) {
        var __path = languages[i].assetsPath;
        // if _assets.json doesn't exist in the specified ` language's path `,
        // create one, with an empty array ` [] `
        if (!fs.existsSync(path.join(__path, '_assets.json')))
            fs.writeFileSync(path.join(__path, '_assets.json'), "[]");
        // path to ` _assets.json ` for each specific language
...
function loadAssets (_assets, asset, lang) {
    return new Promise((resolve, reject) => {
        try {
            if(providerName !== 'FileSystem' && providerName !== 'nedb') {
                db
                .ContentType('_assets')
                .Query()
                .language(lang.code)
...
function getAssetUrl (assetUrl, lang) {
    const relativeUrlPrefix = assetsConf.relative_url_prefix;
...
/**
 * insert or update asset in DB
 * @param  {Object} assets  - Assets of the specified language
 * @param  {Object} asset   - Asset query
 * @param  {String} lang    - Language on which the query operation is to be performed
 * @param  {Boolean} remove - Upsert/Delete the asset
 * @return {Promise}  - Return promise once the operation is complete
 */

function upsert (_assetObject, assets, _path, asset, lang, remove) {
    return new Promise((resolve, reject) => {
        ...
        if (_.isEqual(_assetObject, asset) && !isForceLoad && fs.existsSync(_assetPath)) {
            ...
            helper.downloadAssets(_assetPath, asset, function (error, data) {
                if (error) {
...
helper.getAssets = function () {
    ...
    if(providerName === "FileSystem" || providerName === "nedb") {
        // loads physical location of _assets.json path in the system
        _assets = loadAssetPaths(_assets);
    }

    return (asset, lang, remove, cb) => {
        try {
            var _path = lang.assetsPath,
                assets, _assetObject;

            loadAssets(_assets, asset, lang)
                .then(result => {
                    assets = result;
                    _assetObject = _.find(assets, {'uid': (!remove) ? asset.uid : asset});
                    if(remove) {
                        if (_assetObject) {
                        ...
                        // upsert asset
                        upsert(_assetObject, assets, _path, asset, lang, remove).then(result => {
                            cb(null, result);
                        })
                        .catch(error => {
                            cb(error, null);
...
// delete assets
helper.deleteAssets = function (fileLink, folderLink) {
