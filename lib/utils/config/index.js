...
function Config() {
	try {
		...
		_config.path = {
			base: _dirname,
			theme: _theme,
			templates: ((_config.syncContentOnly === true) && _config.path.templates) || path.join(_theme, 'templates'),
		...
		if(_config.storage && _config.storage.provider && _config.storage.provider.toLowerCase() !== 'filesystem' || _config.storage.provider.toLowerCase() !== 'nedb')
			_config.storage.provider = _config.storage.provider.toLowerCase();
...
function languages(config) {
	...
	while (i < _languages.length) {
		if (_languages[i].code && _languages[i].relative_url_prefix) {
            ...
            if(storage.provider.toLowerCase() === 'filesystem' || storage.provider.toLowerCase() === 'cdn') {
