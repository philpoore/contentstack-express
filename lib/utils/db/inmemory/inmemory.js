...
if(providerName !== 'FileSystem' && providerName !== 'nedb')
    providerInmemory = require(path.join(config.get('path.base'), 'providers', providerName, 'inmemory'));
        ...
        if(this._inmemory) {
            ...
            if (providerName !== 'FileSystem' && providerName !== 'nedb') {
                ...
                for (var l=0, lTotal = languages.length; l < lTotal; l++) {
                    // Prepares the Functions for the Different languages
                    // to be called using Async's parallel function
                    calls[languages[l]['code']] = (function (language){
                        return function (langCallback) {
                            providerInmemory(language, langCallback);
                        }
                    })(languages[l])
                }
                // initiates the process
                async.parallel(calls, function (languageError, languageResult) {
                    if (languageError) {
                        throw languageError;
                    }
                    // this one cached the content_types and _routes inmemory
                    self.cache = languageResult;
                });
            } else {
                this.cache = this.cache || {};
...
Inmemory.prototype.get = function(language, content_type_id, query, includeWrapper) {
    ...
    if(language && content_type_id && this.cache[language]) {
        ...
        if(result && query && typeof query === "object") {
            ...
            if(!includeWrapper && (providerName === "FileSystem" || providerName === "nedb")) result = _.map(result, "_data");
        }
    }
    return result;
};
