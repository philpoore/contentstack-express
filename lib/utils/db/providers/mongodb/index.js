...
MongodbStorage.prototype.insert = function(data, callback) {
    try {
        if (data && typeof data == "object" && data._content_type_uid && data._uid) {
            ...
            if (content_type == "_content_types") insertionData = helper.findReferences(insertionData);
            self.db
                ...
                .count({
                    ...
                    if (result !== 0) {
                        ...
                        self.db.collection(collectionName).insert(insertionData, function (error, result) {
                            ...
                            InMemory.set(language, content_type, data._uid, insertionData);
                            callback(null, 1);
                        });
                    }
                });
            ...
            if(!assetDownloadFlag && contentTypeId === assetRouteName) {
                ...
                async.parallel(calls, function (err, result) {
                    try {
                        ...
                        if (result) {
                            var _data = _.cloneDeep(result.entries);
                            _data = (!remove) ? {"entries": _data} : _data;
                        if (count) _data.count = _data.entries.length;
            ...
            if (Object.keys(_query).length === 2 && contentTypeId && language) {
                self.db
                    ...
                    .remove({"_content_type_uid": contentTypeId, "_lang": language}, function (error, result) {
                        if (error)
                            return callback(error, null);
                        // clears routes of the specified collection
                        self.db
                            .collection('routes')
                            .remove({'content_type.uid': contentTypeId, '_lang': language}, function (error, result) {
                                if(error)
