...
var path          = require('path'),
    ...
    del_keys      = ['created_by', 'updated_by', '_uid', '_data'],
    // Common keys
    assetRoutes   = '_assets',
    cacheRoutes   = '_routes',
...
MongodbStorage.prototype.insert = function (data, callback) {
    try {
        ...
        if (_.isPlainObject(_data) && _data._content_type_uid && _data._uid) {
            // Not maintaining `_routes` in collection
            if (_data._content_type_uid === cacheRoutes)
                return callback(null, 1);

            var collection = (_data._content_type_uid === contentRoutes || _data._content_type_uid === assetRoutes) ? _data._content_type_uid: entryRoutes;
...
/**
 * Function which is used to find a data based on the given query
 *
...
MongodbStorage.prototype.findOne = function (query, callback) {
    try {
        ...
        if (_.isPlainObject(_query) && !_.isEmpty(_query) && _query._content_type_uid && _query._uid) {
            var remove = _query._remove || false,
                ...
                collection = (_query._content_type_uid === contentRoutes || _query._content_type_uid === assetRoutes) ? _query._content_type_uid: entryRoutes,
                // TODO: add user custom projection
                projection = {'_id': 0, '_content_type_uid': 0};

            // Omit unwanted keys
            _query = _.omit(_query, del_keys);

            // Removed _routes collection
            if (collection === '_entries') {
                // TODO: need locale + route at least
                if(_query._content_type_uid === '_routes')
                    delete _query._content_type_uid;
            }
                    ...
                    try {
                        ...
                        if (objects && objects.length) {
                            ...
                            if (includeReferences) {
                                ...
                                self.includeReferences(objects, language, references, parentID, function (error, result) {
                                    if (error)
                                        return callback(error, null);
...
MongodbStorage.prototype.remove = function (query, callback) {
    try {
        ...
        if (_.isPlainObject(_query) && _query._content_type_uid) {
            ...
            if(_query._content_type_uid === cacheRoutes)
                return callback(null, 1);

            var collection = (_query._content_type_uid === contentRoutes || _query._content_type_uid === assetRoutes) ? _query._content_type_uid: entryRoutes,
