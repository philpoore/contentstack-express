...
var datastore       = require('nedb'),
    ...
    skipForms       = [entryRoute, schemaRoute];
...
nedbStorage.prototype.find = function (query, options, callback) {
    try {
        ...
        if (_.isPlainObject(query) && !_.isEmpty(query) && _.isPlainObject(options)) {
            ...
            if (!assetDwldFlag && _query._content_type_uid === assetRoute) {
                var results = InMemory.get(locale, _query._content_type_uid, _query),
                    _data = {assets: (results && results.length) ? results : []}
        ...
        if (_.isPlainObject(data) && !_.isEmpty(data) && data._content_type_uid && data._uid) {
            ...
            fileStorage.insert(data, function (err, result) {
                try {
                    ...
                    if (contentTypeId === "_routes" || contentTypeId === "_content_types" || contentTypeId == assetRoute) {
                        return callback(null, result);
                    } else {
    ...
    try {
        if (_.isPlainObject(data) && !_.isEmpty(data) && data._content_type_uid && data._uid) {
            ...
            fileStorage.upsert(data, function (err, result) {
                if (err) throw err;
                if (data._content_type_uid === "_routes" || data._content_type_uid === assetRoute) {
                    return callback(null, result);
                } else {
    ...
    try {
        if (_.isPlainObject(query) && !_.isEmpty(query)) {
            ...
            fileStorage.remove(query, function (err, result) {
                ...
                // to remove the unwanted keys from query and create reference query
                query = helper.filterQuery(query);
                if (query._content_type_uid === "_routes" || query._content_type_uid === assetRoute) {
                    return callback(null, result);
                } else {
